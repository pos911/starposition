...
const SYSTEM_INSTRUCTION = `당신은 서울에서 20년간 활동 중인 현대적인 맞춤형 점성술사입니다. 특히 대한민국 30대 여성의 커리어, 재테크, 라이프스타일 심리 분석에 정통합니다.
...
function buildFortunePrompt(name: string, birthdate: string, birthtime?: string): string {
  ...
  return `${name}님(${age}세, ${parsed.zodiac}, ${parsed.chineseZodiac})의 오늘(${kstDate} KST 기준) 별자리 운세를 분석해주세요.
...
30대 여성의 삶인 커리어, 재테크, 인간관계, 라이프스타일 맥락에서 분석하고, 2026년 트렌드를 반영하여 아래 JSON으로 응답해주세요:
...
export async function generateFortune(name: string, birthdate: string, birthtime?: string): Promise<FortuneResponse> {
  ...
  const response = await client.models.generateContent({
    ...
    contents: [{ role: 'user', parts: [{ text: buildFortunePrompt(name, birthdate, birthtime) }] }],
    config: {
